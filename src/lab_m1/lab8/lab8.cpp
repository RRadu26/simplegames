...
void Lab8::Init()
{
    ...
    {
        lightPositions[0] = glm::vec3(0, 1, 1);
        lightPositions[1] = glm::vec3(4, 1, 1);
        lightPositions[2] = glm::vec3(-4, 1, 1);
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light_position = glGetUniformLocation(shader->program, "light_positions");
    glUniform3fv(light_position, 3, glm::value_ptr(lightPositions[0]));
    ...
    int spotlights = glGetUniformLocation(shader->program, "spotlight");
    glUniform1i(spotlights, spotlight);

    int angles = glGetUniformLocation(shader->program, "angle");
    glUniform1f(angles, angle);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_K)) angle += 20*deltaTime;
        if (window->KeyHold(GLFW_KEY_L)) angle -= 20*deltaTime;
        if(window->KeyHold(GLFW_KEY_8)) lightDirection -= glm::vec3(0, 0, deltaTime);
        if(window->KeyHold(GLFW_KEY_2)) lightDirection += glm::vec3(0, 0, deltaTime);
        if(window->KeyHold(GLFW_KEY_4)) lightDirection -= glm::vec3(deltaTime, 0, 0);
        if(window->KeyHold(GLFW_KEY_6)) lightDirection += glm::vec3(deltaTime, 0, 0);
