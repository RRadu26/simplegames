...
namespace implemented
{
    class Camera_gen
    {
     public:
        ...
        Camera_gen(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
        {
        ...
        {
            // Translates the Camera_gen using the `dir` vector computed from
            // `forward`. Movement will always keep the Camera_gen at the same
        ...
        {
            // TODO(student): Translate the Camera_gen using the `forward` vector.
        ...
        void TranslateRight(float distance)
        {
            ...
            // You need to translate the Camera_gen using the `right` vector.
            // Usually, however, translation using Camera_gen's `right` vector
            // is not very useful, because if the Camera_gen is rotated around the
            // `forward` vector, then the translation on the `right` direction
            // will have an undesired effect, more precisely, the Camera_gen will
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            glm::vec4 newVector = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 0, 1)) * glm::vec4(up, 1);
        ...
        {
            // TODO(student): Rotate the Camera_gen in third-person mode around
